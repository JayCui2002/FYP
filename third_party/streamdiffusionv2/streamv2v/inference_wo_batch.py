...
class SingleGPUInferencePipeline:
    ...
    def start_stream_session(self, prompt: str, images: torch.Tensor, noise_scale: float) -> tuple[SingleGPUStreamSession, np.ndarray]:
        ...
        # 用最近几帧输入作为刷新种子，避免 session 只靠首帧附近的上下文反复重建。
    ...
    def _refresh_stream_session(self, session: SingleGPUStreamSession, images: torch.Tensor) -> tuple[SingleGPUStreamSession, np.ndarray]:
        """Rebuild the no-batch stream session from the latest frame block before KV refresh boundaries."""
        ...
        refreshed_session.processed = len(self.pipeline.denoising_step_list)
        self.processed = refreshed_session.processed
        return refreshed_session, refresh_video
    ...
    def run_stream_batch(self, session: SingleGPUStreamSession, images: torch.Tensor) -> List[np.ndarray]:
        ...
        for batch_idx in range(input_batch):
            chunk_images = images[:, :, batch_idx * session.chunk_size:(batch_idx + 1) * session.chunk_size]
            if session.current_start // self.pipeline.frame_seq_length >= self.t_refresh:
                session, refresh_video = self._refresh_stream_session(session, chunk_images)
                if refresh_video is not None:
                    outputs.append(refresh_video[[-1]])
                continue
        ...
        session.last_image = images[:, :, [-1]].clone()
    ...
    def run_inference(self, input_video_original: torch.Tensor, prompts: list, 
        ...
        if end_idx <= input_video_original.shape[2]:
            ...
            video = (video * 0.5 + 0.5).clamp(0, 1)
            video = video[0].permute(0, 2, 3, 1).contiguous()
            results[save_results] = video.detach().cpu().float().numpy()
            save_results += 1
