        ...
        if kv_cache is None:
            ...
            for i, c_start in enumerate(current_start):
                ...
                if (current_end > kv_cache["global_end_index"][i].item()) and (
                    ...
                    start_idx = max(0, int(target_end - num_new_tokens))
                    end_idx = min(int(target_end), kv_cache_size)
                    write_len = end_idx - start_idx
                    if write_len > 0:
                        kv_cache["k"][i:i+1, start_idx:end_idx] = roped_key[i:i+1, -write_len:]
                        kv_cache["v"][i:i+1, start_idx:end_idx] = v[i:i+1, -write_len:]

                    local_end_index = kv_cache["local_end_index"][i].item()

                else:
                    current_end_i = int(current_end.item() if hasattr(current_end, "item") else current_end)
                    global_end_i = int(kv_cache["global_end_index"][i].item())
                    delta = max(0, current_end_i - global_end_i)
                    local_end_index = int(kv_cache["local_end_index"][i].item()) + delta
                    local_end_index = min(local_end_index, kv_cache_size)

                    rolling_end = current_end_i + int(num_new_tokens)
                    ...
                    local_start_index = max(0, int(local_start_index))
                    local_end_index = max(local_start_index, int(local_end_index))
                    write_len = local_end_index - local_start_index
                    if write_len > 0:
                        kv_cache["k"][i:i+1, local_start_index:local_end_index] = roped_key[i:i+1, -write_len:]
                        kv_cache["v"][i:i+1, local_start_index:local_end_index] = v[i:i+1, -write_len:]
